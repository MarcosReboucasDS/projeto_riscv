...
int main() {
    std::vector<uint32_t> program = {
        ...
        0x003101B3,  /// ADD x3, x2, x3   => 11
        0x40218233,  /// SUB x4, x3, x2   => 11 - 5 = 6
};

RiscVEmulator emulator;
    emulator.load_program(program);
    emulator.run();
    std::cout << "Result in x3 (ADD): " << emulator.registers[3] << std::endl;  // Should print 11
    std::cout << "----------------------\n";
    std::cout << "Result in x4 (SUB): " << emulator.registers[4] << std::endl;  // Should print 6
    std::cout << "----------------------\n";
